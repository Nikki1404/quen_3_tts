...
async def run_client(server, text, language, speaker, instruct, output, play):
    payload = {
        "text": text,
        "language": language,
        "speaker": speaker,
    ...
    async with connect(
        ...
        connected_ns = now_ns()
        connection_latency_ms = elapsed_ms(connection_start_ns, connected_ns)
        print(f"[connection-latency] {connection_latency_ms:.2f} ms")

        request_start_ns = now_ns()
        await websocket.send(json.dumps(payload, ensure_ascii=False))
        request_sent_ns = now_ns()
        send_call_ms = elapsed_ms(request_start_ns, request_sent_ns)
        ...
        async for message in websocket:
            ...
            if isinstance(message, bytes):
                if first_audio_received_ns is None:
                    ...
                    ttfa = elapsed_ms(request_start_ns, first_audio_received_ns)
                    print(f"[first-audio] {ttfa:.2f} ms")
    ...
    if sample_rate is None:
        raise RuntimeError("Sample rate was not received.")
    ...
    print("=" * 80)
    print(f"Connection latency : {connection_latency_ms:.2f} ms")
    print(f"Send call          : {send_call_ms:.2f} ms")
    print(f"CLIENT TTFB        : {client_ttfb_ms:.2f} ms")
    print(f"CLIENT TTFT/TTFA   : {client_ttft_ms:.2f} ms")
    print(f"Audio -> Done      : {first_audio_to_done_ms:.2f} ms")
    ...
    if server_metrics:
        ...
        print(f"SERVER TTFT/TTFA   : {server_metrics.get('ttft_ms')} ms")
    ...
    if play:
        print()
        print("[play] Playing generated audio...")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Qwen3-TTS WebSocket client with latency metrics and optional playback"
    )

    parser.add_argument(
        "--server",
        default="ws://127.0.0.1:8001/ws/tts",
        help="WebSocket server URL",
    )
    parser.add_argument("--text", required=True, help="Text to synthesize")
    parser.add_argument("--language", default="English", help="Language")
    parser.add_argument("--speaker", default="Aiden", help="Speaker name")
    parser.add_argument("--instruct", default="", help="Voice instruction")
